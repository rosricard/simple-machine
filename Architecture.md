...
`controllers/states.py` defines `SystemState`:
...
| any → ERROR | exception, timeout, unreachable pose |
| ERROR → IDLE | `reset` command |

MainController is responsible for enforcing these transitions; commands arriving
in invalid states are rejected with an error ack.
